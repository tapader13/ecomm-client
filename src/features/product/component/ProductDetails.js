...
export default function ProductDetails() {
  ...
  const cartPrd = useSelector(selectCartItems);
  useEffect(() => {
  ...
  const handleAddToCart = (e) => {
    ...
    if (cartPrd.findIndex((cid) => cid.product.id === product.id) < 0) {
      const newProduct = {
        product: product.id,
        quantity: 1,
      };
      dispatch(addToCartItemsAsync(newProduct));
